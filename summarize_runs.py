...
DEFAULT_CKPT_NAME = "po_lo_joint_adaptive.pth"
...
def _expand_paths(paths: Iterable[str]) -> List[str]:
    ...
    for path in paths:
        if any(ch in path for ch in ["*", "?", "["]):
            expanded.extend(sorted(glob(path)))
            continue
        if os.path.isdir(path):
            expanded.append(os.path.join(path, DEFAULT_CKPT_NAME))
            continue
        expanded.append(path)
    return expanded


def _validate_paths(paths: Iterable[str]) -> List[str]:
    final: List[str] = []
    for path in paths:
...
def main() -> None:
    ...
    ap.add_argument("--checkpoints", nargs="+", required=True, help="paths/dirs/globs for .pth checkpoints")
    ...
    ckpt_paths = _validate_paths(_expand_paths(args.checkpoints))
    ...
    if args.robustness_csv:
        print("\n== robustness summary ==")
        for path in _validate_paths(_expand_paths(args.robustness_csv)):
